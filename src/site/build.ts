...
const publicBasePath = normalizeBasePath(process.env.FORKCAST_PUBLIC_BASE_PATH);
const rawRecordBase = (process.env.FORKCAST_RAW_RECORD_BASE ?? "").replace(/\/+$/g, "");
...
const recordPath = (path: string): string =>
  rawRecordBase && path.startsWith("/records/") ? `${rawRecordBase}/${path.slice("/records/".length)}` : publicPath(path);
...
<head>
  ...
  <link rel="stylesheet" href="${publicPath(`/assets/site.css?v=${SITE_ASSET_VERSION}`)}">
</head>
<body>
  <header class="topbar">
    <a class="brand" href="${publicPath("/")}">Forkcast Data</a>
    <nav>
      <a href="${publicPath("/browser.html")}">Data</a>
      <a href="${publicPath("/search.html")}">Search</a>
      <a href="${publicPath("/snapshots.html")}">Snapshots</a>
      <a href="${publicPath("/schemas.html")}">Schemas</a>
      <a href="${publicPath("/mcp.html")}">MCP</a>
      <a href="${publicPath("/admin/")}">Admin</a>
      <a href="${publicPath("/llms.txt")}">llms.txt</a>
    </nav>
  </header>
  <main>${body}</main>
  <script>window.__FORKCAST_BASE_PATH=${JSON.stringify(publicBasePath)};window.__FORKCAST_RAW_RECORD_BASE=${JSON.stringify(rawRecordBase)};</script>
  <script type="module" src="${publicPath(`/assets/site.js?v=${SITE_ASSET_VERSION}`)}"></script>
</body>
</html>
`;
...
const js = `
const basePath=String(window.__FORKCAST_BASE_PATH||'').replace(/\\/$/,'');
const rawRecordBase=String(window.__FORKCAST_RAW_RECORD_BASE||'').replace(/\\/$/,'');
const hrefFor=(url)=>{
  const value=String(url||'');
  if(!value)return value;
  if(/^[a-z][a-z0-9+.-]*:/i.test(value))return value;
  if(rawRecordBase&&value.startsWith('/records/'))return rawRecordBase+'/'+value.slice('/records/'.length);
...
const resultMarkup=({d,score},queryTerms)=>'<article class="result"><h3><a href="'+hrefFor(d.url)+'">'+escapeHtml(d.title)+'</a></h3><p>'+escapeHtml(snippet(d,queryTerms))+'</p><div class="badges"><span class="badge">'+escapeHtml(d.kind)+'</span><span class="badge">score '+score+'</span>'+((d.tags||[]).slice(0,5).map((tag)=>'<span class="badge">'+escapeHtml(tag)+'</span>').join(''))+'</div><p><a href="'+hrefFor((d.citations&&d.citations[0]&&d.citations[0].url)||d.url)+'">source/provenance</a></p></article>';
const queryParam=(query)=>query?hrefFor('/search.html')+'?q='+encodeURIComponent(query):hrefFor('/search.html');
const focusSearch=()=>{
  const input=document.querySelector('[data-search-input], [data-home-search]');
  if(!input){window.location.href=hrefFor('/search.html');return false;}
  input.focus();
...
if(searchInput&&results){
  let activeKind='all',meta=null,requestSeq=0;
  ...
  const loadJson=async(url)=>{const res=await fetch(hrefFor(url));if(!res.ok)throw new Error('Unable to load '+url);return res.json();};
  ...
  const runFallback=async(query,queryTerms)=>{
    if(count)count.textContent='Search shards unavailable';
    results.innerHTML='<p>Search index shards are not available yet. Try refreshing after the latest deploy finishes.</p>';
  };
  const run=async()=>{
...
export const buildSite = async (distRoot: string): Promise<void> => {
  ...
  const kindOptions = ["all", "eip", "call", "decision", "thread", "topic", "upgrade", "devnet"]
    ...
    .join("");
  const recentCalls = calls.slice(0, 8).map((call) => `<div class="row"><span><a href="${publicPath(call.canonical_json_url)}"><strong>${call.title}</strong></a><br><small>${call.series} #${call.number} · ${call.date}</small></span><small>${call.decisions.length} decisions</small></div>`).join("");
  const recentThreads = threads.slice(0, 8).map((thread) => `<div class="row"><span><a href="${publicPath(thread.canonical_json_url)}"><strong>${thread.title}</strong></a><br><small>${thread.channel ?? "discord"} · ${thread.date ?? ""}</small></span><small>${thread.message_count ?? 0} messages</small></div>`).join("");
