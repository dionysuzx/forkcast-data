...
if(adminForm){const load=(headers={})=>fetch(hrefFor('/api/admin'),{headers}).then(r=>{if(!r.ok)throw new Error('Admin API unavailable on GitHub Pages static hosting');return r.json()}).then(renderAdmin).catch(e=>{if(adminStatus)adminStatus.textContent=String(e);if(adminControls)adminControls.innerHTML='<span class="pill">Serverless admin API requires Cloudflare or another function host</span>'});load();adminForm.addEventListener('submit',(event)=>{event.preventDefault();const data=new FormData(adminForm);const token=String(data.get('token')||'').trim();const password=String(data.get('password')||'').trim();const headers={};if(token)headers.authorization='Bearer '+token;if(password)headers['x-admin-password']=password;window.__adminHeaders=headers;load(headers)})}
`;
